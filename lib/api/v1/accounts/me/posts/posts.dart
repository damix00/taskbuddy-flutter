...
class MyPostsRoute {
  ...
  Future<List<PostResultsResponse>> getBookmarks(String token, {int offset = 0}) async {
    var response = await Requests.fetchEndpoint(
      "${ApiOptions.path}/accounts/me/posts/bookmarks?offset=$offset",
      method: "GET",
      headers: {
        'Authorization': "Bearer ${token}",
      }
    ...
    var posts = <PostResultsResponse>[];

    for (var post in response.response!.data["posts"]) {
      posts.add(PostResultsResponse.fromJson(post));
    }
